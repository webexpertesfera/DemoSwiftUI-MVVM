...
struct FAQs: View {
    @State var faqsobject: FaqsObject = FaqsObject()
    var body: some View {
        ZStack{
            VStack{
                ...
                ScrollView(showsIndicators:false){
                    VStack{
                        if faqsobject.faqs?.count ?? 0 > 0 {
                            
                            if let language1 = UserDefaults.standard.value(forKey: Keys.language) as? String {
                                if language1 == "ar" {
                                    ForEach(0 ..< (faqsobject.faqs?.count ?? 0),id:\.self) { ind in
                                        HStack{
                                            Text(faqsobject.faqs?[ind].title_ar ?? "")
                                        ...
                                        VStack{
                                            Text(faqsobject.faqs?[ind].text_ar ?? "")
                                    ...
                                    ForEach(0 ..< (faqsobject.faqs?.count ?? 0),id:\.self) { ind in
                                        HStack{
                                            Text(faqsobject.faqs?[ind].title ?? "")
                                                .foregroundColor(Color("000000").opacity(0.7))
                                                .font(Font.custom("poppins", size: 18))
                                                .fontWeight(.semibold)
            ...
            .task {
                UserApiUtil.getFaqs(success: { res in
                    faqsobject = res
                }, failure: { _ in
